...
fn = 'CascoBayOADataFALL2016.xlsx'
fpath = os.path.join(parent,fn)


cbep_data = pd.io.excel.read_excel(fpath, header = 0)
...
CO2dict = pyco2.CO2SYS(cbep_data['pco2'], cbep_data['ph'], 4, 3,
                       cbep_data['sal'], cbep_data['temp'],
                       cbep_data['temp'],
                 0,0,0,0,
                 1,9,1)
...
keys_to_extract = ["OmegaARin", "OmegaCAin", 'TAlk', 'TCO2']
replace_keys =['omega_a', 'omega_c', 'ta', 'dic']
...
with open('cbepco2sys_out.csv', 'w') as f:
